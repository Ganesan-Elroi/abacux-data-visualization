...
If the prompt has nothing to do with filtering data, return the word "NONE".
DO NOT format as code block, just text.
DO NOT return SQL comments (lines starting with --).

EXAMPLES:
- "show March 2026 data" → si.invoice_date BETWEEN '2026-03-01' AND '2026-03-31'
- "give all month details" → NONE
- "show paid invoices" → si.paid_status = 1
- "customer named John" → LOWER(c.customer_name) LIKE '%john%'
- "invoices above 5000" → si.grand_total > 5000
- "January to March 2026" → si.invoice_date BETWEEN '2026-01-01' AND '2026-03-31'
...
4. If the user just wants to see "all" data or "details" without specific criteria, return NONE.
5. For date filters, use si.invoice_date with BETWEEN clause in format 'YYYY-MM-DD'.
6. Today's date is {datetime.now().strftime('%Y-%m-%d')} - use this for relative date calculations.
"""
    try:
        ...
        # Remove SQL comments that LLM might return
        if condition.startswith("--") or condition.startswith("#"):
            return ""
        
        # If response indicates no filtering needed, return empty
        if not condition or condition.lower() in ["none", "n/a", "no filter", "no filtering", "empty"]:
            return ""
...
def index():
    ...
    # required fields = base_query, (filters optional), columns (to be shown), custom prompt (User can ask question with existing data - Optional)
    ...
    if extra_where:
        ...
        print("No additional SQL filter applied (showing all data)")
